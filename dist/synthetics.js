...
function seleneseCommands() {
  ...
  return {
    ...
    setWindow: function SetWindow() {
      logger.log(__STEP__, "__LOG_MSG__", "__TC_NAME__");
      return $browser.manage().window().setSize(__WIDTH__, __HEIGHT__).then(val => {
        return val;
      });
    },
    click: function Click() {
      logger.log(__STEP__, "__LOG_MSG__", "__TC_NAME__");
      return $browser.waitForAndFindElement(__LOCATOR__, DefaultTimeout).then(el => {
        el.click();
    ...
    select: function Select() {
      logger.log(__STEP__, "__LOG_MSG__", "__TC_NAME__");
      return $browser.waitForAndFindElement(__LOCATOR__, DefaultTimeout).then(function (el1) {
        el1.click();
        logger.log(__STEP__, "__LOG_MSG2__", "__TC_NAME__");
        return $browser.waitForAndFindElement(__LOCATOR2__, DefaultTimeout).then(function (el2) {
          el2.click();
          return Promise.resolve(true);
        });
      });
    ...
    waitForElementEditable: function WaitForElementEditable() {
      logger.log(__STEP__, "__LOG_MSG__", "__TC_NAME__");
      return $browser.waitForAndFindElement(__LOCATOR__, __TIMEOUT__).then(el => {
        return el;
      });
    },
    ...
    waitForElementNotPresent: function WaitForElementNotPresent() {
      logger.log(__STEP__, "__LOG_MSG__", "__TC_NAME__");
      return $browser.waitForAndFindElement(__LOCATOR__, __TIMEOUT__).then(el => {
        return $browser.wait($driver.until.elementIsNotVisible(el)).then(isNotVisible => {
          assert(isNotVisible);
          return Promise.resolve(true);
        });
      });
    ...
    answerOnNextPrompt: function AnswerOnNextPrompt() {
      logger.log(__STEP__, "__LOG_MSG__", "__TC_NAME__");
      return $browser.switchTo().alert().sendKeys("__TEXT__").then(el => {
        alert.accept();
        return Promise.resolve(true);
      });
    },
    ...
    dragAndDrop: function DragAndDrop() {
      logger.log(__STEP__, "__LOG_MSG__", "__TC_NAME__");
      return $browser.waitForAndFindElement(__LOCATOR__, DefaultTimeout).then(draggedEle => {
        return $browser.waitForAndFindElement(__LOCATOR2__, DefaultTimeout).then(droppedEle => {
          logger.log(__STEP__, "__LOG_MSG2__", "__TC_NAME__");
    ...
    editContent: function EditContent() {
      logger.log(__STEP__, "__LOG_MSG__", "__TC_NAME__");
      return $browser.waitForAndFindElement(__LOCATOR__, DefaultTimeout).then(e => {
